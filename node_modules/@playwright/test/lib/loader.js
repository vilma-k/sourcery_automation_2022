...
var _hostPlatform = require("playwright-core/lib/utils/hostPlatform");
 ...
 */
const defaultTimeout = 30000; // To allow multiple loaders in the same process without clearing require cache,
...
class Loader {
  ...
  async _processConfigObject(config, configDir) {
    ...
    validateConfig(this._configFile || '<default config>', config); // 2. Override settings from CLI.
    ...
    config.use = { ...(0, _util.filterUndefinedFixtures)(config.use),
      ...(0, _util.filterUndefinedFixtures)(this._configCLIOverrides.use)
    };

    for (const project of config.projects || []) this._applyCLIOverridesToProject(project); // 3. Resolve config.
    ...
    const throwawayArtifactsPath = packageJsonDir || process.cwd(); // Resolve script hooks relative to the root dir.

    if (config.globalSetup) config.globalSetup = resolveScript(config.globalSetup, configDir);
    if (config.globalTeardown) config.globalTeardown = resolveScript(config.globalTeardown, configDir); // Resolve all config dirs relative to configDir.
    ...
    if (config.screenshotsDir !== undefined) config.screenshotsDir = path.resolve(configDir, config.screenshotsDir);
  ...
  buildFileSuiteForProject(project, suite, repeatEachIndex, filter) {
    if (!this._projectSuiteBuilders.has(project)) this._projectSuiteBuilders.set(project, new ProjectSuiteBuilder(project, this._fullConfig.projects.indexOf(project)));
  ...
  _applyCLIOverridesToProject(projectConfig) {
    ...
    projectConfig.use = { ...(0, _util.filterUndefinedFixtures)(projectConfig.use),
      ...(0, _util.filterUndefinedFixtures)(this._configCLIOverrides.use)
    };
  }
  ...
  _resolveProject(config, fullConfig, projectConfig, throwawayArtifactsPath) {
    ...
    if (projectConfig.screenshotsDir !== undefined) projectConfig.screenshotsDir = path.resolve(this._configDir, projectConfig.screenshotsDir);
    ...
    const screenshotsDir = takeFirst(projectConfig.screenshotsDir, config.screenshotsDir, path.join(testDir, '__screenshots__', process.platform, name));
    return {
      ...
      use: { ...(0, _util.filterUndefinedFixtures)(config.use),
        ...(0, _util.filterUndefinedFixtures)(projectConfig.use)
      }
    };
  }
...
class ProjectSuiteBuilder {
  ...
  _cloneEntries(from, to, repeatEachIndex, filter, relativeTitlePath) {
    for (const entry of from._entries) {
      if (entry instanceof _test.Suite) {
        const suite = entry._clone();

        to._addSuite(suite); // Ignore empty titles, similar to Suite.titlePath().
        ...
        test.retries = this._project.retries; // We rely upon relative paths being unique.
        // See `getClashingTestsPerSuite()` in `runner.ts`.

        test._id = `${(0, _utils.calculateSha1)(relativeTitlePath + ' ' + entry.title)}@${entry._requireFile}#run${this._index}-repeat${repeatEachIndex}`;
...
const workers = _hostPlatform.hostPlatform.startsWith('mac') && _hostPlatform.hostPlatform.endsWith('arm64') ? cpus : Math.ceil(cpus / 2);
...
function folderIsModule(folder) {
  ...
  if (!packageJsonPath) return false; // Rely on `require` internal caching logic.
